...
public class InfoGUI {
	...
	// public static final int xPosition = ControlGUI.xPosition;
	// public static final int yPosition = ySize ;

	public static final int xPosition = ControlGUI.xPosition + ControlGUI.xSize;
	public static final int yPosition = ControlGUI.yPosition;
	...
	private void configureGui() {
		...
		for (int i = 0; i < 3; i++) {
			...
			JProgressBar int_neg = new JProgressBar();
			int_neg.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			int_neg.setForeground(colorExt);
			tcpTorquePanel.add(int_neg, gbc);
			wrenchBarsTcp.add(int_neg);

			JProgressBar int_pos = new JProgressBar();
			int_pos.setForeground(colorExt);
	...
	private SensorForExternalForce sensorForceExt = SunriseConnector.getRobot().getSensorForExternalForce();
	private ForceSensorData extForce;

	private LBR robot = SunriseConnector.getRobot();
						...
						for (int i = 0; i < 7; i++) {
							if (measuredTorque[i] < 0) {
								torqueBarsInt.get(2 * i).setValue(
										(int) (-1 * measuredTorque[i] / SunriseConnector.MAX_JOINT_TORQUES[i] * 100));
								torqueBarsInt.get(2 * i + 1).setValue(0);
								torqueBarsInt.get(2 * i).setString(fmt.format(measuredTorque[i]));
								torqueBarsInt.get(2 * i + 1).setString("");
							} else {
								torqueBarsInt.get(2 * i).setValue(0);
								torqueBarsInt.get(2 * i + 1).setValue(
										(int) (measuredTorque[i] / SunriseConnector.MAX_JOINT_TORQUES[i] * 100));
								torqueBarsInt.get(2 * i).setString("");
								torqueBarsInt.get(2 * i + 1).setString(fmt.format(measuredTorque[i]));
							}
							torqueBarsInt.get(2 * i).setString("");
							torqueBarsInt.get(2 * i + 1).setString("");
							torqueValuesInt.get(i).setText(fmt.format(measuredTorque[i]));
							// torqueValuesInt.get(i).setText("");
							if (measuredTorque[i] >= 0)
								torqueValuesInt.get(i).setForeground(Color.BLUE);
							else
								torqueValuesInt.get(i).setForeground(Color.RED);
						}
						...
						for (int i = 0; i < 7; i++) {
							if (extTorque[i] < 0) {
								torqueBarsExt.get(2 * i).setValue(
										(int) (-1 * extTorque[i] / SunriseConnector.MAX_JOINT_TORQUES[i] * 100));
								torqueBarsExt.get(2 * i + 1).setValue(0);
								torqueBarsExt.get(2 * i).setString(fmt.format(extTorque[i]));
								torqueBarsExt.get(2 * i + 1).setString("");
							} else {
								torqueBarsExt.get(2 * i).setValue(0);
								torqueBarsExt.get(2 * i + 1).setValue(
										(int) (extTorque[i] / SunriseConnector.MAX_JOINT_TORQUES[i] * 100));
								torqueBarsExt.get(2 * i).setString("");
								torqueBarsExt.get(2 * i + 1).setString(fmt.format(extTorque[i]));
							}
							torqueBarsExt.get(2 * i).setString("");
							torqueBarsExt.get(2 * i + 1).setString("");
							torqueValuesExt.get(i).setText(fmt.format(extTorque[i]));
							// torqueValuesExt.get(i).setText("");
							if (extTorque[i] >= 0)
								torqueValuesExt.get(i).setForeground(Color.BLUE);
							else
								torqueValuesExt.get(i).setForeground(Color.RED);
						}

						// external forces
						extForce = robot.getExternalForceTorque(SunriseConnector.getTool().getDefaultMotionFrame());
						...
						for (int i = 0; i < 6; i++) {
							if (extForceD[i] < 0) {
								wrenchBarsTcp.get(2 * i).setValue(
										(int) (-1 * extForceD[i] / SunriseConnector.TCP_FORCES[i] * 100));
								wrenchBarsTcp.get(2 * i + 1).setValue(0);
							} else {
								wrenchBarsTcp.get(2 * i).setValue(0);
								wrenchBarsTcp.get(2 * i + 1).setValue(
										(int) (extForceD[i] / SunriseConnector.TCP_FORCES[i] * 100));
							}
							wrenchValuesTcp.get(i).setText(fmt.format(extForceD[i]));
							wrenchBarsTcp.get(2 * i + 1).setString("");
							wrenchBarsTcp.get(2 * i).setString("");
							if (extForceD[i] >= 0)
								wrenchValuesTcp.get(i).setForeground(Color.BLUE);
							else
								wrenchValuesTcp.get(i).setForeground(Color.RED);
						}
